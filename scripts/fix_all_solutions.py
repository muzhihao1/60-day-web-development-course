...
def fix_solution_file(filepath):
    # Extract day number from filepath
    day_match = re.search(r'day-(\d+)\.md', filepath)
    if not day_match:
        print(f"Error: Could not extract day number from {filepath}")
        return False
    ...
    if fm_start == -1 or fm_end == -1:
        print(f"Error: Could not find frontmatter boundaries in {filepath}")
        return False
    ...
    # Check if files are in the old format (with name: and code:)
    if 'name:' in files_content and 'code:' in files_content:
        # Extract each file
        file_entries = []
        current_file = None
        current_desc = None
    ...
    if extensions_match:
        ...
        modified = True
        print(f"  - Fixed extensions format")
...
solution_files = glob.glob('/Users/liasiloam/Vibecoding/web dev course/src/content/solutions/day-*.md')
solution_files.sort()
...
for filepath in solution_files:
    if fix_solution_file(filepath):
        fixed_count += 1

print("=" * 50)
print(f"Processed {fixed_count}/{len(solution_files)} files successfully")